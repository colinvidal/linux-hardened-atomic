...
static void rxrpc_resend(struct rxrpc_call *call)
{
	...
	_enter("{%d,%d,%d,%d},",
	       ...
	       atomic_read_wrap(&call->sequence),
	...
	for (loop = call->acks_tail;
		...
		if (sp->need_resend) {
			...
			sp->hdr.serial = atomic_inc_return_wrap(
					&call->conn->serial);
...
{
	rxrpc_rotate_tx_window(call, atomic_read_wrap(&call->sequence));
}
...
process_further:
	...
	case RXRPC_PACKET_TYPE_ACK:
		...
		tx = atomic_read_wrap(&call->sequence);
...
send_ACK_with_skew:
	ack.maxSkew = htons(atomic_read_wrap(&call->conn->hi_serial) -
			    ntohl(ack.serial));
send_ACK:
	...
	serial = atomic_inc_return_wrap(&call->conn->serial);
